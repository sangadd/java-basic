...
public class StaticOrganize {
    ...
    /*
    ***************** 엄청 중요한 개념 정리 *****************
    name과 count 둘 다 멤버 변수이다.
    멤버변수(필드)는 static 이 붙은 것과 아닌 것에 따라 분류 가능
    ...
    static 이 붙은 멤버 변수는 인스턴스와 무관하게 클래스에 바로 접근해서 사용할 수 있고
    클래스 자체에 소속되어 있다.
    따라서 클래스 변수라 한다.
    클래스 변수는 자바 프로그램을 시작할 때 딱 1개가 만들어진다.
    인스턴스와는 다르게 보통 여러곳에서 공유하는 목적으로 사용한다.
    ...
    메서드가 종료되면 스택 프레임도 제거되는데 이때 스택 프레임에 포함된 지역 변수도 함께 제거된다.
    ...
    3. 클래스 변수
    클래스 변수는 메서드 영역의 static 영역에 보관되는 변수다.
    메서드 영역은 프로그램 전체에서 사용하는 공용 공간이다.
    클래스 변수는 해당 클래스가 JVM에 로딩 되는 순간 생성된다.
    그리고 JVM이 종료될 때까지 생명주기가 이어진다.
    따라서 가장 긴 생명주기를 가진다.

    * static 이 정적이라는 이유는 바로 여기에 있당
    힙 영역에 생성되는 인스턴스 변수는 동적으로 생성되고 제거됨
